	...
	entries.forEach(entry => {
		if (entry.isIntersecting) {
			if (entry.target.id === 'avatar') {
				...
				fadeIn(entry.target);
			} else if (entry.target.id === 'about_me_container' || entry.target.id === 'portfolio_container' || entry.target.id === 'contact_section') {
				fadeIn(entry.target);
			}
		}
	});
...
document.querySelectorAll('#name, #avatar, #dealing, #about_me_container, #portfolio_container, #contact_section').forEach(element => {
